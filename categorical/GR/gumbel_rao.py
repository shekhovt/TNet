...
def conditional_gumbel(logits, D, k=1):
    ...
    # iid. exponential
    E = torch.distributions.exponential.Exponential(rate=torch.ones_like(logits),validate_args=False).sample([k]) # [k, *, C]
    ...
    # adjusted = (D * (-torch.log(Ei) + torch.log(Z)) +(1 - D) * -torch.log(E/torch.exp(logits) + Ei / Z))
    # adjusted = D * (-torch.log(Ei) + logZ) \
    #    + (1 - D) * -torch.log(E/torch.exp(logits) + Ei / Z)
    logE = torch.log(E) # = -G
    logEi = torch.log(Ei) # = -Gi
    adjusted = D * (-logEi + logZ) + (1-D) * -log_sum_exp2(logE - logits, logEi - logZ)
    check_real(adjusted)
    return adjusted - logits


@torch.no_grad()
def conditional_gumbel1(logits, D, k=1):
    """Outputs k samples of Q = StandardGumbel(), such that argmax(logits
    + Q) is given by D (one hot vector).
    logits [*, K]
    D [*, K] -- one-hots
    k -- number of samples
    output: [k, *, K] -- samples of G | D
    """
    assert(D.shape == logits.shape)
    logits = torch.log_softmax(logits, dim=-1) # normalzie
    U = logits.new_empty(k, *logits.shape).uniform_()
    G = -torch.log(-torch.log(U)) # Gumbel
    logits = logits.unsqueeze(0)
    D = D.unsqueeze(0)
    GD = (G * D).sum(dim=-1, keepdim=True) # repeat selected Gk along all categories
    # G_cond = D * G  - (1-D)*log_sum_exp2(-G-logits, -GD) - logits
    G_cond = D * (G-logits) - (1-D)*log_sum_exp2(-G, logits-GD)
    return G_cond
...
def exact_conditional_gumbel(logits, D, k=1):
    ...
    while len(gumbels) < k:
        ...
        if logits.add(gumbel).argmax(dim=-1) == idx:
...
# %%
if __name__ == 'main':
    R = exact_conditional_gumbel(torch.tensor([[1.0,2.0, 5.0]]), torch.tensor([[1.0, 0.0, 0.0]]), k=10000)
    print(R.mean(dim=0),end=' ')
    print(R.std(dim=0))
    # tensor([[0.9952, 1.2695, 0.8132]])
    R = conditional_gumbel(torch.tensor([[1.0,2.0, 5.0]]), torch.tensor([[1.0, 0.0, 0.0]]), k=10000)
    print(R.mean(dim=0), end=' ')
    print(R.std(dim=0))

    R = conditional_gumbel1(torch.tensor([1.0,2.0, 5.0]), torch.tensor([1.0, 0.0, 0.0]), k=10000)
    print(R.mean(dim=0), end=' ')
    print(R.std(dim=0))
